...
SELECT SUM(items.items_price - (items.items_price*items.items_discount/100)) as itemstotalprice , COUNT(cart.cart_itemsid) as countitems , cart.* , items.*,items.items_price - (items.items_price*items.items_discount/100) as itemspricediscount FROM cart
